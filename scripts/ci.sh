...
step "1c. RFC 064 explicit unsafe runtime boundary"
...
if [ "$V13_FAIL" -eq 0 ] && [ "$V13_PASS" -eq 77 ]; then
  pass "v0.13-test: $V13_PASS checks (explicit ack + RFC 064 authority + RFC 065 Lean boundary) all PASS"
else
  cat /tmp/v13_ci.txt
  fail "v0.13-test: expected 77 passes; $V13_FAIL failed, $V13_PASS passed"
fi
