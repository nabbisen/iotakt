...
* **Pipelining-correct buffered reads** — `readFromBuffer` parses one
...
def testBufferedRead : IO Unit := do
  IO.println ""
  IO.println "=== B. readFromBuffer pipelining ==="
  ...
  if client < 0 then check "socketpair (pipelining)" false
    ...
    let (r1, rest1) ← readFromBuffer server ByteArray.empty 8192 30
    ...
    let (r2, rest2) ← readFromBuffer server rest1 8192 30
    let p2 := match r2 with | .request req => req.path | _ => "?"
    check "pipelined request 2 path = /2" (p2 == "/2")

    let (r3, rest3) ← readFromBuffer server rest2 8192 30
  ...
  if c2 < 0 then check "socketpair (CL + pipeline)" false
    ...
    let reqs :=
      ...
      "GET /next HTTP/1.1\r\nHost: x\r\n\r\n"
    let rb := reqs.toUTF8
    let _ ← Io.send c2 rb 0 rb.size
    Socket.closeFdRaw (fd32 c2)

    let (r1, rest1) ← readFromBuffer s2 ByteArray.empty 8192 30
    match r1 with
    | .request req =>
        check "CL request body = 'HELLO'" ((String.fromUTF8? req.body |>.getD "") == "HELLO")
    | _ => check "CL request parsed" false
    let (r2, _) ← readFromBuffer s2 rest1 8192 30
    let p2 := match r2 with | .request req => req.path | _ => "?"
