...
### Request-size limits (`readFull` / `readFromBuffer`)
...
### Pipelining-correct buffered reads (`readFromBuffer`)
...
`readFromBuffer` carries a leftover buffer:
...
`AB`. Its only contact with the OS is through `Iotakt.Server`; that is the
point — to confirm the handoff surface is sufficient for a real server,
without iotakt becoming one.
...
primitives (`readFull`, `readFromBuffer`), size limits, the `Iotakt.Server`
handoff surface.
