...
**iotakt v0.11 — RFC 014 — henret v0.15.2**
...
with `lake build IotaktProofs`. The CI gate runs **321 executable checks**
across 13 test suites plus the live server smoke tests.
...
| Theorem | Property |
|---------|----------|
| `inject_ok_of_mailbox` | when the owner mailbox exists, Henret `inject` always returns `.ok` — the formal mitigation of Henret's `inject` mailbox precondition (holds v0.6.0 → v0.15.2) |
...
### Lifecycle, framing, and stabilization (v0.5 – v0.11, against real Henret v0.15.2)
...
| Henret `RuntimeState`/`StepResult`/`inject`/`Envelope` unchanged v0.11.0 → v0.15.2 | Henret source (diffed each bump) |
