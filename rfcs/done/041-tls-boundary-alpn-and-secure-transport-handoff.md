...
- **Intended phase:** jemmet/TLS integration
...
This RFC clarifies that TLS, ALPN, certificates, SNI, and secure transport policy are not implemented inside iotakt. It defines the minimal handoff boundaries needed so that jemmet or a future TLS layer can use iotakt safely.
...
jemmet sees:    HTTP semantics, routing, request/response lifecycle
```
...
```text
1. Accepted TCP connection is registered to a jemmet/TLS actor.
...
7. Once handshake completes, jemmet HTTP parsing begins over decrypted bytes.
```
...
- jemmet integration RFCs may depend on this boundary without moving TLS into iotakt.
